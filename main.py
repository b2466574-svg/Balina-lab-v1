...
# ══ V2.8 — EŞİKLER KAZANAN ARALIĞINA GÖRE AYARLANDI ═══════════════════════
# Defterdeki 12 TP4 kazananının (MFE ≥ %20) ÖLÇÜLEN aralıkları:
#     skor      35.1 – 59.4   (medyan 55.8)
#     RSI LONG  38.8 – 79.6
#     RSI SHORT 35.2 – 69.2
# Eski eşikler (skor≥65, RSI LONG≤40, SHORT≥70) bu 12 kazananın 12'sini de
# eliyordu: 173 sinyalden geriye 2 sinyal, 0 kazanan kalıyordu.
...
#  LABORATUVAR MODU
#
#  AMAÇ: Filtrelerin işe yarayıp yaramadığını ÖLÇMEK. Şu anki botta sadece
#  filtreden GEÇEN sinyalleri görüyoruz; bu yüzden "sweep şartı işe yarıyor
#  mu?" sorusunu asla cevaplayamayız — kesilen sinyallerin ne yapacağını
#  hiç görmüyoruz (seçilim yanlılığı).
#
#  ÇÖZÜM: Hiçbir kapı sinyali ENGELLEMEZ. Her kapı sadece "geçer/kalır"
#  diye KAYDEDİLİR. 20 gün / 2.000 kapanış sonunda /kazanan komutu her
#  kapının gerçek EV katkısını gösterir. Ayarı BOT DEĞİL, HASAN yapar.
#
#  TASARIM KARARLARI (ve gerekçeleri):
#   • Telegram'a tek tek sinyal GÖNDERİLMEZ — günde binlerce olacak.
#     Sadece SQLite'a yazılır, kilometre taşlarında özet gelir.
#   • Aynı coinde aynı anda tek pozisyon: aksi halde aynı fikir defalarca
#     sayılır ve örneklem şişer (temmuzdaki TRUST ×4 hatası).
#   • İKİ giriş popülasyonu: TETİK anında bir, RETEST onayında bir.
#     Böylece "retest beklemek işe yarıyor mu?" doğrudan ölçülür.
#   • RASTGELE kontrol grubu: aynı coinlerde yazı-turayla açılan işlemler.
#     "Bulduğum edge gerçek mi, çıkış modelinin ürettiği yanılsama mı?"
#     sorusunu ancak bu cevaplar.
#   • Kapı bayrakları giriş ANINDA donar, sonradan güncellenmez.
# ============================================================================ #
...
def lab_db():
    """Laboratuvar veritabanı. Ana bottan AYRI dosya — karışmasın."""
    global _lab_db
    if not LAB_MODE:
        return None
    if _lab_db is not None:
        return _lab_db
    try:
        yol = LAB_DB_PATH
        ...
        if d and not os.path.isdir(d):
            ...
            except Exception: yol = "lab.db"
        con = sqlite3.connect(yol, check_same_thread=False)
        con.execute("""CREATE TABLE IF NOT EXISTS lab(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            acilis_ts REAL, kapanis_ts REAL, sembol TEXT, yon TEXT,
            giris_tipi TEXT,          -- TETIK | RETEST | RASTGELE
            setup TEXT, yapi_tipi TEXT,
            giris REAL, stop REAL, tp1 REAL, tp2 REAL, tp3 REAL,
            -- ÖZELLİKLER (giriş anında donmuş)
            skor REAL, arastirma REAL, rsi REAL,
            trend_15m TEXT, trend_1h TEXT, trend_4h TEXT, rejim_1d TEXT,
            btc_1h TEXT, btc_4h TEXT,
            sweep INT, fib_derinlik REAL, hacim_orani REAL, spread REAL,
            aralik_konum REAL, oi REAL, funding REAL,
            cvd REAL, cvd_n INT, balina REAL, balina_n INT, spoof TEXT,
            grafik_puan REAL, haber REAL, saat INT, gun INT,
            -- KAPI BAYRAKLARI: 1 = geçerdi, 0 = kesilirdi
        ...
        logger.error("LAB veritabanı açılamadı: %s", e)
        _lab_db = None
    return _lab_db


def lab_yeni() -> Dict[str, Any]:
    """Boş özellik/bayrak kaydı. Kapılar geçtikçe doldurulur."""
    d: Dict[str, Any] = {}
...
def lab_kaydet(sig: Dict[str, Any], lab: Dict[str, Any], giris_tipi: str) -> Optional[int]:
    con = lab_db()
    if not con:
        return None
    try:
        ak = sig.get("akis") or {}
        c = ak.get("cvd") or {}; w = ak.get("whale") or {}; sp = ak.get("spoof") or {}
        rs = sig.get("research") or {}
        gr = rs.get("grafik") or {}
        yapi = str(sig.get("structure") or "")
        ...
        cur = con.execute(
            ...
            (time.time(), sig.get("symbol"), sig.get("direction"), giris_tipi,
             ...
             int(safe_float(lab.get("sweep"))), safe_float(sig.get("fib_depth")),
             safe_float(lab.get("hacim_orani")), safe_float(sig.get("spread_bps")),
             safe_float(lab.get("aralik_konum")), safe_float(sig.get("oi_change_pct")),
             safe_float(sig.get("funding")), safe_float(c.get("oran")),
             int(safe_float(c.get("n"))), safe_float(w.get("baski")),
        ...
        logger.warning("LAB kayıt hatası: %s", e)
        return None
...
def lab_kapat(pos: Dict[str, Any], R: float, outcome: str) -> None:
    ...
    try:
        ...
        stats["lab_kapanis"] = n
        if n % LAB_MILESTONE == 0:
            asyncio.create_task(lab_kilometre(n))
    except Exception as e:
        logger.debug("LAB kapanış hatası: %s", e)


async def lab_kilometre(n: int) -> None:
    con = lab_db()
    if not con:
        return
    try:
        r = con.execute("SELECT COUNT(*), AVG(CASE WHEN r>0 THEN 100.0 ELSE 0 END), AVG(r) "
                        "FROM lab WHERE sonuc!='ACIK'").fetchone()
        rnd = con.execute("SELECT COUNT(*), AVG(r) FROM lab "
                          "WHERE sonuc!='ACIK' AND giris_tipi='RASTGELE'").fetchone()
        kalan = max(0, LAB_TARGET_CLOSES - n)
        await safe_send_telegram(
            f"🧪 LABORATUVAR — {n:,} kapanış\n"
            f"Genel: WR%{safe_float(r[1]):.1f} EV={safe_float(r[2]):+.3f}R\n"
            f"Rastgele kontrol: n={int(safe_float(rnd[0]))} EV={safe_float(rnd[1]):+.3f}R\n"
            f"Hedefe kalan: {kalan:,}\n"
            + ("📊 Hedef doldu — /kazanan ile raporu al." if kalan == 0 else
               "⚠️ Henüz KARAR VERME. Rapor için /kazanan (n<2000 iken sadece ön izleme)."))
    except Exception as e:
        logger.debug("LAB kilometre hatası: %s", e)
...
# V2.7 — SKOR TAVANI. Defterdeki 12 TP4 kazananının EN YÜKSEK skoru 59.4.
# V10_MIN_QUALITY=65 kapısı bu 12 kazananın 12'sini de eler (p=0.072).
# Skor↔MFE korelasyonu 4 ölçümde de sıfır/hafif NEGATİF (son: r=-0.075, n=466);
# skor bandı tablosunda ort. MFE monoton AZALIYOR (<45: %3.81 → 75+: %2.60).
# Bu yüzden skoru ALT sınır olarak kullanmak sinyal değil GÜRÜLTÜ seçiyor.
...
def v10_structure_gate(symbol, k1h, k4h, erken=False):
    ...
    for side in ("LONG", "SHORT"):
        ...
        _sw = v10_detect_sweep(side, k, ms)
        _LAB_GATE["sweep_deger"] = _sw
        if V126_REQUIRE_SWEEP and _sw < 1.0:
...
async def analyze_v10_symbol(symbol: str) -> Optional[Dict[str, Any]]:
    symbol = normalize_symbol(symbol)
    v131_say("tarandi")
    lab = lab_yeni()          # LAB: kapı bayrakları burada toplanır
    ...
    # V2.8: KURULUM TİPİ KAPISI. Defterde SÜPÜRME DÖNÜŞÜ 0/16 kazanan (0.00x lift),
    # CHoCH 5/45 (1.60x), BOS 7/112 (0.90x). n küçük olduğu için LAB'da yine
    # kaydedilir, sadece g_yapi=0 işaretlenir — sonradan doğrulanabilsin diye.
    if V10_BLOK_YAPI:
        _y = str(gate.get("why") or "")
        _ytip = ("SÜPÜRME DÖNÜŞÜ" if "Süpürme" in _y else
                 "CHoCH" if "CHoCH" in _y else "BOS" if "BOS" in _y else "")
        if _ytip and _ytip.upper() in V10_BLOK_YAPI:
            stats["v10_red_yapitip"] = int(stats.get("v10_red_yapitip", 0)) + 1
            v131_say("yapi_tip")
            lab["g_yapi"] = 0
    ...
    if V126_WAIT_RETEST:
        ...
        if lvl_r > 0:
            ...
            if durum != "ONAY":
                ...
                # LAB: TETİK anında (KURULDU) gir — retest beklemenin değerini
                # ölçmek için "beklemeden giren" popülasyon lazım. BEKLIYOR /
                # FAKEOUT / SURE_DOLDU durumlarında zaten TETİK'te girilmişti,
                # tekrar girilmez.
                if not (LAB_MODE and durum == "KURULDU"):
                    return None
                lab["g_retest"] = 0
                lab["giris_tipi"] = "TETIK"
                retest_not = f"LAB: retest beklenmeden girildi ({acik})"
            else:
                lab["giris_tipi"] = "RETEST"
            if durum == "ONAY":
                retest_not = acik
    ...
    if not akis_ok:
        ...
        # LAB: hangi alt kapının kestiğini ayrı ayrı işaretle
        if "CVD" in akis_red or "akış" in akis_red: lab["g_cvd"] = 0
        elif "balina" in akis_red: lab["g_balina"] = 0
        elif "spoof" in akis_red: lab["g_spoof"] = 0
        else: lab["g_cvd"] = 0
        if not LAB_MODE: return None
    ...
    # V2.8: RSI kapısı artık İKİ TARAFLI ve kazanan aralığına göre.
    # Eski tek taraflı hali (LONG r>40 ELE / SHORT r<70 ELE) 12 kazananın
    # 11'ini kesiyordu. Kazananların gerçek RSI'si LONG 38.8–79.6,
    # SHORT 35.2–69.2 aralığına yayılmış durumda.
    _rsi_red = ((side == "LONG" and not (V10_RSI_LONG_MIN <= r <= V10_RSI_LONG_MAX)) or
                (side == "SHORT" and not (V10_RSI_SHORT_MIN <= r <= V10_RSI_SHORT_MAX)))
    if _rsi_red:
    ...
    if score < V10_MIN_QUALITY:
        ...
        lab["g_skor"] = 0
        if not LAB_MODE: return None
    # V2.7: SKOR TAVANI. Defterde 12 TP4 kazananının en yükseği 59.4 skorluydu;
    # yüksek skor bandı hem TP1 isabetinde hem ort. MFE'de en kötüsü. Bu kapı
    # "çok yüksek skorlu" sinyalleri eler. 0 = kapalı.
    if V10_MAX_QUALITY > 0 and score > V10_MAX_QUALITY:
        stats["v10_red_tavan"] = int(stats.get("v10_red_tavan", 0)) + 1
        v131_say("skor_tavan")
        lab["g_skor"] = 0
        if not LAB_MODE: return None
    ...
    return {"symbol":symbol,"direction":side,"entry":entry,"strategy":"V10_SMC",
            ...
            "gunluk_rejim":gunluk,"gunluk_mesafe":gunluk_mes,"lab":lab,
...
def v10_open_paper(sig):
    ...
    mp["open"].append({
        ...
        "db_id":sig.get("db_id"),"lab_id":sig.get("lab_id"),
...
async def maybe_send_v10_signal(sig):
    ...
    if block:
        ...
        lab["g_tekrar"] = 0
        # LAB'da tek istisna: AYNI COİNDE AÇIK POZİSYON varsa yine de girme.
        # Aksi halde aynı fikir defalarca sayılır, örneklem şişer ve
        # bağımsızlık varsayımı çöker (temmuzdaki TRUST ×4 hatası).
        if (not LAB_MODE) or "açık pozisyon" in block:
            return
...
async def lab_rastgele_loop() -> None:
    """RASTGELE KONTROL GRUBU.
    Aynı coin evreninde, yazı-turayla yön seçip aynı TP/SL ile pozisyon açar.
    Neden şart: '%55 isabet buldum' demek tek başına anlamsızdır — aynı çıkış
    modeliyle rastgele giriş de %55 verebilir. Gerçek edge = strateji EV'si
    eksi rastgele EV'si. Bu grup olmadan hiçbir sonuç yorumlanamaz."""
    if not (LAB_MODE and LAB_RANDOM_PCT > 0):
        return
    await asyncio.sleep(90)
    while True:
        try:
            havuz = [c for c in list(COINS)]
            if havuz and len(_v10_mem().get("open", [])) < LAB_MAX_OPEN:
                sym = normalize_symbol(random.choice(havuz))
                if not any(p.get("symbol") == sym for p in _v10_mem().get("open", [])):
                    k = await get_klines(sym, V122_STRUCT_TF, 60)
                    k = _s_closed(k)
                    if len(k) >= 30:
                        side = random.choice(("LONG", "SHORT"))
                        ob = await v10_fetch_orderbook(sym)
                        px = safe_float(ob.get("mid")) or closes(k)[-1]
                        if px > 0:
                            tgt = v10_targets(side, px, atr(k, V10_ATR_PERIOD)[-1], None)
                            sig = {"symbol": sym, "direction": side, "entry": px,
                                   "stop": tgt["stop"], "tp1": tgt["tp1"], "tp2": tgt["tp2"],
                                   "tp3": tgt["tp3"], "tp1_rr": tgt["tp1_rr"],
                                   "tp2_rr": tgt["tp2_rr"], "tp3_rr": tgt["tp3_rr"],
                                   "score": 0.0, "event": "RASTGELE",
                                   "structure": "RASTGELE KONTROL",
                                   "setup": "RASTGELE", "rsi": round(rsi(closes(k))[-1], 1),
                                   "candle_ts": str(k[-1][0]),
                                   "spread_bps": safe_float(ob.get("spread_bps"))}
                            sig["lab_id"] = lab_kaydet(sig, lab_yeni(), "RASTGELE")
                            v10_open_paper(sig)
...
def _lab_p(ev1: float, n1: int, ev2: float, n2: int, sd: float) -> float:
    """İki grubun EV farkı için iki taraflı p-değeri (normal yaklaşım)."""
    if n1 < 2 or n2 < 2 or sd <= 0:
        return 1.0
...
async def cmd_kazanan(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """LABORATUVAR RAPORU — hangi kapı gerçekten kazandırıyor?

    Her kapı için: kapıyı GEÇENLER vs KESİLENLER karşılaştırılır.
    Kapı işe yarıyorsa geçenlerin EV'si belirgin yüksek olmalı.
    Fark küçükse kapı boşuna sinyal kesiyor demektir."""
    con = lab_db()
    if not con:
        await update.message.reply_text("🧪 LAB veritabanı yok (LAB_MODE=false?)"); return
    L = ["🧪 LABORATUVAR RAPORU"]
    try:
        g = con.execute("SELECT COUNT(*), AVG(CASE WHEN r>0 THEN 100.0 ELSE 0 END), "
                        "AVG(r), SUM(r) FROM lab WHERE sonuc!='ACIK'").fetchone()
        n = int(safe_float(g[0]))
        acik = con.execute("SELECT COUNT(*) FROM lab WHERE sonuc='ACIK'").fetchone()[0]
        L.append(f"Kapanan: {n:,} / hedef {LAB_TARGET_CLOSES:,} | açık: {acik:,}")
        if n == 0:
            L.append("\nHenüz kapanış yok."); await update.message.reply_text("\n".join(L)); return
        L.append(f"Genel: WR%{safe_float(g[1]):.1f} EV={safe_float(g[2]):+.3f}R "
                 f"toplam {safe_float(g[3]):+.2f}R")

        # standart sapma — p hesapları için
        sd = safe_float(con.execute(
            "SELECT AVG((r-m)*(r-m)) FROM lab, (SELECT AVG(r) m FROM lab WHERE sonuc!='ACIK') "
            "WHERE sonuc!='ACIK'").fetchone()[0]) ** 0.5 or 1.2

        # 1) RASTGELE KONTROL — her şeyin ölçüldüğü taban
        rg = con.execute("SELECT COUNT(*), AVG(r) FROM lab "
                         "WHERE sonuc!='ACIK' AND giris_tipi='RASTGELE'").fetchone()
        st = con.execute("SELECT COUNT(*), AVG(r) FROM lab "
                         "WHERE sonuc!='ACIK' AND giris_tipi!='RASTGELE'").fetchone()
        rn, rev = int(safe_float(rg[0])), safe_float(rg[1])
        sn, sev = int(safe_float(st[0])), safe_float(st[1])
        L.append(f"\n━━ TABAN ÇİZGİSİ ━━")
        L.append(f"Rastgele giriş : n={rn:<5,} EV={rev:+.3f}R")
        L.append(f"Strateji       : n={sn:<5,} EV={sev:+.3f}R")
        if rn >= 30 and sn >= 30:
            p = _lab_p(sev, sn, rev, rn, sd)
            L.append(f"GERÇEK EDGE    : {sev-rev:+.3f}R  (p={p:.4f}"
                     + (" ✅ anlamlı)" if p < 0.01 else " ❌ anlamsız)"))
        else:
            L.append("⚠️ taban için en az 30+30 kapanış gerekiyor")

        # 2) KAPI KAPI ANALİZ — asıl soru
        L.append(f"\n━━ KAPILAR (geçen vs kesilen) ━━")
        kapilar = [("g_sweep","Sweep şartı"),("g_retest","Retest bekleme"),
                   ("g_cvd","Gerçek CVD"),("g_balina","Balina gözü"),
                   ("g_spoof","Spoof bloğu"),("g_hacim","Hacim kapısı"),
                   ("g_btc","BTC uyumu"),("g_rsi","RSI kapısı"),
        ...
        for kol, ad in kapilar:
            a = con.execute(f"SELECT COUNT(*), AVG(r) FROM lab WHERE sonuc!='ACIK' "
                            f"AND giris_tipi!='RASTGELE' AND {kol}=1").fetchone()
            b = con.execute(f"SELECT COUNT(*), AVG(r) FROM lab WHERE sonuc!='ACIK' "
                            f"AND giris_tipi!='RASTGELE' AND {kol}=0").fetchone()
            na, eva = int(safe_float(a[0])), safe_float(a[1])
            nb, evb = int(safe_float(b[0])), safe_float(b[1])
            if nb == 0:
                sonuc.append((0.0, f"{ad:<20} — hiç kesilmemiş, ölçülemez")); continue
            if na < LAB_MIN_CELL or nb < LAB_MIN_CELL:
                sonuc.append((0.0, f"{ad:<20} n={na}/{nb} ⚠️ yetersiz (min {LAB_MIN_CELL})"))
                continue
            fark = eva - evb
            p = _lab_p(eva, na, evb, nb, sd)
            isaret = "✅" if (p < alfa and fark > 0) else ("🔴" if (p < alfa and fark < 0) else "⚪")
            sonuc.append((abs(fark) if p < alfa else 0.0,
                          f"{isaret} {ad:<18} geçen {eva:+.3f}R (n={na:,}) | "
                          f"kesilen {evb:+.3f}R (n={nb:,}) | fark {fark:+.3f} p={p:.4f}"))
        for _, satir in sorted(sonuc, key=lambda x: -x[0]):
            L.append(satir)
        ...
        for r in con.execute("SELECT giris_tipi, COUNT(*), "
                             ...
                             "FROM lab WHERE sonuc!='ACIK' GROUP BY 1 ORDER BY 4 DESC"):
            L.append(f"{str(r[0]):<10} n={int(safe_float(r[1])):<5,} "
                     f"WR%{safe_float(r[2]):.1f} EV={safe_float(r[3]):+.3f}R")

        # 4) REJİM KONTROLÜ — sonuç hava durumu mu?
        L.append(f"\n━━ REJİM KONTROLÜ (sonuç havaya mı bağlı?) ━━")
        for r in con.execute("SELECT btc_4h, COUNT(*), AVG(r) FROM lab "
                             "WHERE sonuc!='ACIK' AND giris_tipi!='RASTGELE' "
                             "GROUP BY 1 ORDER BY 3 DESC"):
            L.append(f"BTC 4H {str(r[0]):<6} n={int(safe_float(r[1])):<5,} EV={safe_float(r[2]):+.3f}R")
        L.append("→ İki rejimde de aynı yönde sonuç varsa edge gerçek. "
                 "Sadece birinde varsa rejim etkisidir, kural yapma.")

        # 5) DÖNEM KONTROLÜ — ilk yarı / ikinci yarı
        orta = con.execute("SELECT AVG(acilis_ts) FROM lab WHERE sonuc!='ACIK'").fetchone()[0]
        if orta and n >= 200:
            L.append(f"\n━━ ÖRNEKLEM DIŞI KONTROL ━━")
            for ad, kos in (("İlk yarı", "<"), ("İkinci yarı", ">=")):
                r = con.execute(f"SELECT COUNT(*), AVG(r) FROM lab WHERE sonuc!='ACIK' "
                                f"AND giris_tipi!='RASTGELE' AND acilis_ts {kos} ?",
                                (orta,)).fetchone()
                L.append(f"{ad:<12} n={int(safe_float(r[0])):<5,} EV={safe_float(r[1]):+.3f}R")
            L.append("→ İki yarı benzer değilse bulgular kalıcı değil.")

        if n < LAB_TARGET_CLOSES:
            L.append(f"\n⚠️ {n:,}/{LAB_TARGET_CLOSES:,} — HENÜZ KARAR VERME. "
                     f"Bu bir ÖN İZLEME; hedefe ulaşmadan ayar yapma.")
        else:
            L.append(f"\n✅ Hedef doldu ({n:,}). Yukarıdaki ✅ kapılar tutulur, "
                     f"⚪ kapılar gevşetilir, 🔴 kapılar kaldırılır.")
    except Exception as e:
...
async def cmd_kazanan(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Her kapı için tek soru: bu kapı açık olsaydı, KAZANANLARIN kaçı elenirdi?
    Kazananları eleyen bir kapı, ne kadar mantıklı görünürse görünsün, zarardır.

    Ölçüt: mfe_pct >= V11_TP4_PCT (yani %20+ gördü). Bu, R muhasebesinden
    bağımsızdır ve açık pozisyonları da doğru sayar."""
    con = lab_db()
    if not con:
        await update.message.reply_text("🧪 LAB veritabanı yok (LAB_MODE=false?)")
        return
    hedef = safe_float(V11_TP4_PCT)
    L = [f"🏆 KAZANAN PROFİLİ (MFE ≥ %{hedef:g})", ""]
    try:
        kaz = con.execute(
            f"SELECT sembol,yon,skor,rsi,oi,mfe_pct,giris_tipi,yapi_tipi,setup "
            f"FROM lab WHERE giris_tipi<>'RASTGELE' AND mfe_pct >= {hedef} "
            f"ORDER BY mfe_pct DESC").fetchall()
        tum_n = int(safe_float(con.execute(
            "SELECT COUNT(*) FROM lab WHERE giris_tipi<>'RASTGELE'").fetchone()[0]))
        if len(kaz) < 5:
            await update.message.reply_text(
                f"🏆 Kazanan sayısı {len(kaz)} — profil çıkarmak için en az 5 gerekir.")
            return
        L.append(f"Kazanan: {len(kaz)} / {tum_n} sinyal (%{len(kaz)/tum_n*100:.1f})")
        L.append("")
        sk = sorted(safe_float(r[2]) for r in kaz if safe_float(r[2]) > 0)
        if sk:
            L.append("📊 SKOR")
            L.append(f"   Kazananlar : {min(sk):.1f} – {max(sk):.1f} (medyan {sk[len(sk)//2]:.1f})")
            L.append(f"   Alt kapı   : V10_MIN_QUALITY = {V10_MIN_QUALITY:g}")
            elenen = sum(1 for x in sk if x < V10_MIN_QUALITY)
            isaret = "🔴" if elenen > len(sk) * 0.5 else ("⚠️" if elenen else "✅")
            L.append(f"   {isaret} Bu kapı kazananların {elenen}/{len(sk)}'ini ELER")
            if V10_MAX_QUALITY > 0:
                el2 = sum(1 for x in sk if x > V10_MAX_QUALITY)
                L.append(f"   Üst kapı   : V10_MAX_QUALITY = {V10_MAX_QUALITY:g} "
                         f"→ {el2}/{len(sk)} eler")
            L.append("")
        L.append("📊 RSI")
        for y in ("LONG", "SHORT"):
            rs = sorted(safe_float(r[3]) for r in kaz if r[1] == y)
            if not rs:
                continue
            kapi = V10_RSI_LONG_MAX if y == "LONG" else V10_RSI_SHORT_MIN
            el = (sum(1 for x in rs if x > V10_RSI_LONG_MAX) if y == "LONG"
                  else sum(1 for x in rs if x < V10_RSI_SHORT_MIN))
            isaret = "🔴" if el > len(rs) * 0.5 else ("⚠️" if el else "✅")
            yon_ad = "≤" if y == "LONG" else "≥"
            L.append(f"   {y:<6} {min(rs):.1f} – {max(rs):.1f}  | kapı {yon_ad}{kapi:g}")
            L.append(f"   {isaret} kazananların {el}/{len(rs)}'ini ELER")
        L.append("")
        L.append("📊 KIRILIM (kazanan oranı / lift)")
        taban = len(kaz) / tum_n if tum_n else 0
        for sut, ad in (("giris_tipi", "Giriş"), ("yapi_tipi", "Yapı"), ("yon", "Yön")):
            rows = con.execute(
                f"SELECT {sut}, COUNT(*), SUM(CASE WHEN mfe_pct>={hedef} THEN 1 ELSE 0 END) "
                f"FROM lab WHERE giris_tipi<>'RASTGELE' GROUP BY {sut}").fetchall()
            L.append(f"   {ad}:")
            for k, n, t in rows:
                n = int(safe_float(n)); t = int(safe_float(t))
                if n < 15:
                    continue
                lift = (t / n) / taban if taban and n else 0
                im = "✅" if lift > 1.3 else ("🔴" if lift < 0.7 else "➖")
                L.append(f"     {im} {str(k):<16} {t:>3}/{n:<4} %{t/n*100:>4.1f}  {lift:.2f}x")
        L.append("")
        L.append("📐 lift 1.00 = ortalama. 1.30+ değerli, 0.70- zararlı.")
        L.append("⚠️ Az sayıda kazananla eşik ayarlamak AŞIRI UYDURMA riskidir.")
        L.append("   Bir kapıyı ancak kazananların yarıdan fazlasını eliyorsa değiştir.")
    except Exception as e:
        L.append(f"Hata: {e}")
    metin = "\n".join(L)
    for i in range(0, len(metin), 3800):
